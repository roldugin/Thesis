...
\item several combinators fuse to a single loop
\item must generate parts of the same loop
\end{itemize}

\subsection{Parts of a loop}
\begin{itemize}
\item Init
\item Guard
\item Body
\item Bottom
\item Done
\item Proceed by showing an exaple, slowly populating the loop
\item Identify problem of identifying common variables
\item Identify the problem of naming conventions
\end{itemize}
...
\begin{itemize}
\item Loop still carries the common structure of having a guard, body, etc..
\item How do we generalise it to a procedural language
\item Identify relaxed constructs
\item Introduce simple liveness analysis
\end{itemize}
