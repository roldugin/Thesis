...
We do not need to delve into too much detail here, since by the time we get to the backend of \DPH (where the fusion takes place) we are no longer interested in how it's done. One important intuition to develop can be illustrated by the following:
...
For every such function the vectoriser generates its \*lifted* version @f^@ thus:
...
In the generated code\footnote{The code generated internally to be used with the backend library of primitive operations} @f^@ becomes the following\footnote{This is no longer true for functions as simple as \code{f} \cite{vectavoid} but it is still a clear example indicative of \DPH inner workings }:

\begin{hscode}
f^ :: Array Float -> Array Float -> Array Float
f^ x = zipWith (+) (zipWith (*) x x)
                   (replicate n 1)
  where n = length x
\end{hscode}

The above code is an example of the code which is compiled against the backend library of flat array combinators. Since the backend library is where array fusion ultimately takes place this type of code will show up quite regularly throughout this thesis.
...
One distinctive feature of \DPH compared to other {\EDSL}s for data parallelism is that is allows one to use user defined data types. We have already seen how a user could define a data type for @Tree@. Likewise we could define a new data type @Shape@ with two constructors:
...
\item A pari of arrays @([:Int:],[:Int:])@ that sore dimensions of all rectanges
