...
The work on \DPH was originally inspired by Blelloch's pioneering work on \name{NESL} \cite{BCH+}, a research language designed to explore the new approach to nested data parallelism\index{data parallelism: nested}{}.
...
The right of Figure \ref{fig:TreeRepr} shows the same tree with each of its levels placed into a \term{nested parallel array}\index{nested: parallel array}{}.
...
\item A \*selector*\index{selector}{} array [:Int:] which determines which of the two constructors has been used. The selector array allows to recreate the original interleaving of values constructed with different constructors.
\end{itemize}
...
By the time the backend is reached in the compilation pipeline the nested data parallelism\index{data parallelism: nested}{} in the original user program has been completely transformed to flat data parallelism:
\begin{itemize}
\item The program is now expressed in terms of flat and \*segmented array combinators*\isegmemented{} that take the segment descriptor as an additional argument.\todo{Some of these segmented operations are shows in listing...}
...
\label{Lit:DPH-fusion-levels}\DPH system originally introduced fusion at two levels:
\begin{description}
\item Removing synchronisation points \hfill \\

Due to the use of \Haskell threads \cite{Jones08atutorial}, the code responsible for distributing the processing across the processing elements\ipe{} introduces many thread @fork/join@ points. This prohibits the application of array fusion that we discussed in Section \ref{sec:Fusion}. Thus if there is no processing to be done between the @join@ and the next @fork@, these are fused together using the rewrite rules\irwrules{} \cite{PTH01}.

\item Stream Fusion \hfill \\

Stream fusion\istreamfusion{} receives the most attention as far as the fusion in \DPH goes. It not only considerably affects the internal implementation of primitive operations in \DPH and in the underlying \name{Vector} library\footnote{http://hackage.haskell.org/package/vector}, but also relies on a certain set of optimisations in the compiler. It will be described in a later section. Like the fusion that removes superfluous synchronisation points, Stream Fusion one also heavily relies on inlining and rewriting to work.
\end{description}
