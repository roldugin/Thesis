...
We will now revisit the example from the previous section and introduce the process by which a \Loop is generated:

This time we extend the example that specifies the user functions with which the @map@ and @filter@ combinators are instantiated. The function @toPercents@ presented on converts an array fractions to their percentage equivalents, filtering out those below $0.01$ (or $1\%$)

passed to the combinators. Let us take a simple function which adjusts elements of an array by applying a certain function $f$ and then picks out elements which are above a certain threshold, say 100. In Haskell the function would have looked something like the following: $>= 0.01$
