...
\subsubsection{Different length inputs}

The result list of @zipWith@ from \Haskell's standard library has the same length as the shortest of its inputs. In the \Loop language this could easily be expressed by setting the output length to be the minimum of the input array lengths and adjusting the iteration range appropriately:
...
\subsubsection{Skipping elements in the inputs}

\LiveFusion has support for fusing loops which do not produce
The code generated in the previous step will produce correct results for the specific problem. However, it will not work in the general case. Suppose we introduce a @filter@ into the combinator pipeline producing input of @zipWith@:
