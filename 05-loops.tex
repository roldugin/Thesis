...
All of the combinators discussed so far were consumed their inputs in the sequential matter from beginning to end. However, there are cases in \DPH and otherwise, where elements need to be picked from the array at random. In particular, the @backpermute@ combinator creates a new array by picking elements at given indices:
...
\todo{insert bpermute codez}

The \*rate system* of the \Loop language makes it easy to switch between sequential and random accessing since the sequential looping statements are not inserted into the loop until the very end.
...
\caption{\Loop code for \code{fold_s (+) 0 segd data}.}
...
\caption{\Loop code for \code{scanl_s (+) 0 segd data}.}
