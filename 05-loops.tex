...
Most of these combinators are also \*point-wise consumers* (Sections \ref{sec:zipping} and \ref{sec:DPH-pointwise-consumers}). They consume values from multiple arrays in lockstep in order to generate an array segment in the inner loop. An example of @replicate_s@ application is shown in Figure~\ref{fig:replicate-s}.
...
As mentioned previously, \FlowFusion cannot currently fuse segmented combinators although this is not a fundamental limitation and it is likely to be lifted in the future.
...
The recent work on ``Combinators for impure yet hygienic code generation'' \cite{HygienicCodeGen} may prove useful to avoid these problems statically.
